...
def get_instagram():
    ...
    engine = sqlalchemy.create_engine('mysql://%(user)s:%(pass)s@%(host)s' % database) # connect to server
    engine.execute('use %s' % database['name']) # select new db
    ...
    # query_url = 'https://api.instagram.com/v1/tags/streetart/media/recent?access_token=%s&max_tag_id=%s' % (instaconfig.instagram['access_token'], max_tag_id)
    query_url = 'https://api.instagram.com/v1/tags/streetart/media/recent?access_token=%s' % (instaconfig.instagram['access_token'])
